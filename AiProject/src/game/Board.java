...
public class Board extends JPanel {
	...
	public boolean shouldDisplay_m = false;
	...
	// board[row][column]
	/*
	 * cols 1 2 3 4 5 6 7 8 rows 1 - - - - - - - - 2 - - - - - - - - 3 - - - - -
	 * - - - 4 - - - - - - - - 5 - - - - - - - - 6 - - - - - - - - 7 - - - - - -
	 * - - 8 - - - - - - - -
	 */
	public Color[][] board = new Color[boardSize][boardSize];

	public Board(boolean shouldDisplay) {
	...
	public Board(Board oldBoard) {
		...
		shouldDisplay_m = false;
	}

	public void graphicsInit() {
		 JFrame frame = new JFrame("Reversi"); frame.add(this);
	...
	public boolean isMoveLegal(Color color, int row, int column) {
		...
		for (int i = row - 1; i >= 0; i--) {
			if (isEmpty(board[i][column])) {
				break;
			} else if (!colorEquals(board[i][column], color)) { // other persons
																// disk
				surrounding = true;
		...
		for (int i = column + 1; i < boardSize; i++) {
			if (isEmpty(board[row][i])) {
				break;
			} else if (!colorEquals(board[row][i], color)) { // other persons
																// disk
				surrounding = true;
			} else { // same color disk
				if (surrounding) { // surrounds other players disks
	...
	public void paint(Graphics g) {
		...
		g2d.setColor(black);
		for (int i = 0; i <= boardSize; i++) {
			g2d.drawLine(initial_x + i * diskSize, initial_y, initial_x + i
					* diskSize, initial_y + 8 * diskSize);
			if (i < boardSize)
				g2d.drawString(Integer.toString(i + 1), initial_x + i
						* diskSize + diskSize / 2, initial_y - diskSize / 4);
		}
		for (int i = 0; i <= boardSize; i++) {
			g2d.drawLine(initial_x, initial_y + i * diskSize, initial_x + 8
					* diskSize, initial_y + i * diskSize);
			if (i < boardSize)
				g2d.drawString(Integer.toString(i + 1), initial_x - diskSize
						/ 2, initial_y + i * diskSize + diskSize * 3 / 4);
		}
		...
		for (int i = 0; i < boardSize; i++) {
			for (int j = 0; j < boardSize; j++) {
				...
				case BLACK:
					g2d.setColor(black);
					g2d.fillOval(initial_x + (i * diskSize) + 1, initial_y
							+ (j * diskSize) + 1, diskSize - 2, diskSize - 2);
					break;
				case WHITE:
					g2d.setColor(white);
					g2d.fillOval(initial_x + (i * diskSize) + 1, initial_y
